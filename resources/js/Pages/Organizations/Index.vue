<template>
  <div>
    <Head :title="__('menu.organizations')" />
    <h1 class="mb-8 text-3xl font-bold">{{__('menu.organizations')}}</h1>
    <div class="flex items-center justify-between mb-6">
      <search-filter v-model="form.search" class="mr-4 w-full max-w-md" @reset="reset">
        <label class="block text-gray-700">{{__('common.trashed')}}:</label>
        <select v-model="form.trashed" class="form-select mt-1 w-full">
          <option :value="null" />
          <option value="with">{{__('common.with_trashed')}}</option>
          <option value="only">{{__('common.only_trashed')}}</option>
        </select>
      </search-filter>
      <Link class="btn-indigo" href="/organizations/create">
        <span class="hidden md:inline">{{__('organizations.create_title')}}</span>
      </Link>
    </div>
      ...
      <table class="w-full whitespace-nowrap">
        <thead>
          <tr class="text-left font-bold">
            <th class="pb-4 pt-6 px-6">{{__('common.name')}}</th>
            <th class="pb-4 pt-6 px-6">{{__('common.city')}}</th>
            <th class="pb-4 pt-6 px-6">{{__('common.phone')}}</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr v-if="organizations.data.length === 0">
            <td class="px-6 py-4 border-t" colspan="4">{{__('organizations.not_found')}}</td>
          </tr>
        </tbody>
      </table>
    </div>
